  ...
  options: [
    {
      ...
      correct: true,
      explanation: `Correct! This implementation uses strong cryptographic practices: random salt per message, PBKDF2 with high iteration count, AES-256-GCM for authenticated encryption, and a master key from environment variables. Each message gets unique encryption parameters.`
    },
    // Real MITRE demonstrative examples as wrong answers
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: Fixed salt simplifying attacks. Using the same salt for all messages enables rainbow table attacks and makes brute force/dictionary attacks more efficient.'
    },
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: 8-bit key vulnerable to brute force. A single byte key provides only 256 possible values, making exhaustive key search trivial.'
    },
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: One-to-one character mapping. Simple substitution ciphers are vulnerable to frequency analysis and can be broken through cryptanalysis.'
    },
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: Short key vulnerable to brute force. A 4-character key provides limited keyspace that can be exhaustively searched.'
    },
    {
      code: `return message.split('').reverse().join('').split('').map((c, i) => String.fromCharCode(((c.charCodeAt(0) - 32 + (i % 95)) % 95) + 32)).join('');`,
      correct: false,
      explanation: 'MITRE pattern: Reversible algorithm. String reversal combined with character shifting is deterministic and easily reversed.'
    },
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: Predictable PRNG for encryption. Linear congruential generators produce deterministic sequences that can be predicted once the seed is known.'
    },
    {
    ...
    },
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: 16-bit key vulnerable to brute force. With only 65,536 possible keys, this keyspace can be exhaustively searched very quickly.'
    },
    {
      ...
      correct: false,
      explanation: 'MITRE pattern: ROT cipher (one-to-one mapping). Rotation ciphers can be easily broken through frequency analysis or by trying all possible rotation values.'
    }
  ]
}